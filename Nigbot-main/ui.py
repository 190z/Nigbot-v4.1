...
class HudCanvas(QWidget):
    def __init__(self, face_path: str, assistant_name: str = "Nigbot", parent=None):
...
class LogWidget(QTextEdit):
    ...
    def __init__(self, parent=None):
        ...
        self._pos     = 0
        self._tag     = "sys"
        self._ai_name_lc = "nigbot"   # updated when assistant name changes
        self._tmr = QTimer(self)
        self._tmr.timeout.connect(self._step)
    ...
    def _next(self):
        ...
        elif tl.startswith(_ai_pfx) or tl.startswith("nigbot:"): self._tag = "ai"
...
class FileDropZone(QWidget):
    ...
    def _browse(self):
        path, _ = QFileDialog.getOpenFileName(
            self, "Select a file for NIGBOT™", str(Path.home()),
            "All Files (*.*);;"
...
class SetupOverlay(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        layout.addWidget(_lbl("Configure Nigbot. .", 9, color=C.PRI_DIM))
        layout.addSpacing(6)
...
class CustomizeOverlay(QWidget):
    ...
    def __init__(self, assistant_name="NIGBOT™", user_name="",
    ...
    def _save(self):
        name = self._name_input.text().strip() or "NIGBOT™"
...
class ClipboardPanel(QWidget):
    """Floating panel shown when text is copied — offers quick NIGBOT™ actions."""
...
class RemoteKeyOverlay(QWidget):
    ...
    def mark_connected(self) -> None:
        ...
        self._timer_lbl.setText("Phone connected — NIGBOT™ ready")
...
class MainWindow(QMainWindow):
    ...
    def __init__(self, face_path: str):
        ...
        self._assistant_name: str = (_cfg.get("assistant_name") or "NIGBOT").strip()
        ...
        self.setWindowTitle(f"{_display} — Log's Industrie's ™ AI Assistant")
        ...
        self.on_interrupt      = None   # callable: () -> None — stop NIGBOT™ mid-speech
        self._muted            = False
        self._current_file: str | None = None
    ...
    # ------------------------------------------------------------------
    @staticmethod
    def _build_nigbot_icon(out_path: Path) -> bool:
        """
        Render a NIGBOT™ arc-reactor icon at 4× resolution and downsample
    ...
    def _create_lnk_windows(lnk: str, target: str, args: str,
        ...
        try:
            ...
            sc.Description      = "Nigbot AI Assistant"
        ...
        try:
            ...
            # ── macOS — proper .app bundle (no Terminal window) ───────────────
                ...
                (app / "Contents" / "Info.plist").write_text(
                    ...
                    '  <key>CFBundleExecutable</key><string>NIGBOT</string>\n'
                    '  <key>CFBundleIdentifier</key>'
                    '<string>com.nigbot.assistant</string>\n'
                    '  <key>CFBundleName</key><string>NIGBOT™</string>\n'
    ...
    def _build_header(self) -> QWidget:
        ...
        lay.addWidget(_badge("NIGBOT™", C.PRI_DIM))
        lay.addSpacing(8)
        ...
        _sub_text = ("Intelligent Nigga"
                     if _disp in ("NIGBOT™", "NIGBOT")
    ...
    def _check_autostart(self) -> bool:
        ...
        try:
            if _OS == "Windows":
                ...
                try:
                    winreg.QueryValueEx(key, "NIGBOT_AI")
                    return True
                ...
                return (Path.home() / "Library" / "LaunchAgents"
                        / "com.nigbot.assistant.plist").exists()
            else:
                return (Path.home() / ".config" / "autostart" / "nigbot.desktop").exists()
        except Exception:
            return False
    ...
    def _toggle_autostart(self):
        ...
        try:
            ...
            if _OS == "Windows":
                ...
                if currently_on:
                    winreg.DeleteValue(reg, "NIGBOT_AI")
                else:
                    pythonw = Path(sys.executable).parent / "pythonw.exe"
                    exe = str(pythonw if pythonw.exists() else sys.executable)
                    winreg.SetValueEx(reg, "NIGBOT_AI", 0, winreg.REG_SZ,
                                      f'"{exe}" "{script}"')
                winreg.CloseKey(reg)
            elif _OS == "Darwin":
                plist_dir = Path.home() / "Library" / "LaunchAgents"
                plist_dir.mkdir(parents=True, exist_ok=True)
                plist = plist_dir / "com.nigbot.assistant.plist"
                if currently_on:
                    ...
                    plist.write_text(
                        ...
                        '"http://www.apple.com/DTDs/PropertyList-1.0.dtd">\n'
                        '<plist version="1.0"><dict>\n'
                        '  <key>Label</key><string>com.nigbot.assistant</string>\n'
    ...
    def _open_customize(self):
        ...
        cw = self.centralWidget()
        ov = CustomizeOverlay(
            cfg.get("assistant_name", "NIGBOT") or "NIGBOT",
            cfg.get("user_name", ""),
    ...
    def _apply_name_update(self, name: str, user_name: str, ui_color: str = ""):
        ...
        self._assistant_name = name.strip() or "NIGBOT"
        display = self._assistant_name.upper()
        self.setWindowTitle(f"{display} — NIGBOT™")
        self._title_lbl.setText(display)
        if display in ("NIGBOT", "NIGBOT™"):
            self._sub_lbl.setText("Next-Gen Intelligent Bot")
        else:
    ...
    def _on_setup_done(self, key: str, os_name: str):
        ...
        if self._overlay:
            ...
            self._overlay = None
        self._apply_state("LISTENING")
        self._assistant_name = _read_full_config().get("assistant_name", "NIGBOT™") or "NIGBOT™"
